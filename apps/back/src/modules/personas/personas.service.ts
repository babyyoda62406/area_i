...
export class PersonasService {
    ...
    async deleteIndicador(personaId: number, indicadorId: number) {
        const tempPersona = await this.getPersona(personaId)
        const tempIndicador = await this.svIndicador.getIdnicador(indicadorId)

        const existeIndicador = tempPersona.Indicadores.some(
            (indicador) => indicador.id === tempIndicador.id
        )


        if(!existeIndicador)throw new HttpException(`El indicador ${tempIndicador.nombre} no existe en esta persona` , HttpStatus.NOT_FOUND)


        tempPersona.Indicadores = tempPersona.Indicadores.filter((Indicador)=> Indicador.id != tempIndicador.id)

        await this.dbPersona.save(tempPersona)

        return {message: 'Indicador eliminado' , personaId , indicadorId}
    }
}
